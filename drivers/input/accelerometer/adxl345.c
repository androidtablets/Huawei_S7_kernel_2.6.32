...
static int gs_adi_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	   unsigned long arg)
{
...
{	
	int ret;
	ret = reg_write(this_gs_data,GS_ADI_REG_OFSX,0x0);   /* set  OFSX offset value*/
	if (ret < 0) {
		printk(KERN_DEBUG "reg_write  GS_ADI_REG_OFSX failed\n");
	}
	ret = reg_write(this_gs_data,GS_ADI_REG_OFSY,0x0);	 /* set OFSY offset value*/
	if (ret < 0) {
		printk(KERN_DEBUG "reg_write  GS_ADI_REG_OFSY failed\n");
	}

	ret = reg_write(this_gs_data,GS_ADI_REG_OFSZ,0x0);	/* set OFSZ offset value*/
...
static void gs_work_func(struct work_struct *work)
{
	...
	*/
	...
	//printk("DEVID = 0x%x\n", devid);
	//printk("DATAX0 = 0x%x DATAX1 = 0x%x DATAY0 = 0x%x DATAY1 = 0x%x DATAZ0 = 0x%x DATAZ1 = 0x%x\n", 
	//dataX_low, dataX_high, dataY_low, dataY_high, dataZ_low, dataZ_high);
	...
	if (!data_is_err)	
	{
		...
		//printk(KERN_ERR "RAW DATA:\n");
		//printk(KERN_ERR "x = %d y = %d z = %d\n", x, y, z);
		...
		#endif
		/*End Modefied by z00175749 20110105*/
...
{	
	...
	devid =reg_read(gs,GS_ADI_REG_DEVID);		
	if ( devid <0 )
		goto err_detect_failed;
	//printk("DEVID = 0x%x\n", devid);

	ret = reg_write(gs,GS_ADI_REG_OFSX,0x0);
	if ( ret <0 )
		goto err_detect_failed;
	ret = reg_write(gs,GS_ADI_REG_OFSY,0x0);
	if ( ret <0 )
		goto err_detect_failed;
	ret = reg_write(gs,GS_ADI_REG_OFSZ,0x0);
	if ( ret <0 )
		goto err_detect_failed;
	
	ret =  reg_write(gs,GS_ADI_REG_BW,0x09);    /* Rate: 100Hz, IDD: 130uA */
    if ( ret <0 )
		goto err_detect_failed;
	ret = reg_write(gs,GS_ADI_REG_DATA_FORMAT,0x0B);  /* Data Format: 16g right justified  	256=1g  16g*/
	if ( ret <0 )
		goto err_detect_failed;
	...
	ret = reg_write(gs,GS_ADI_REG_POWER_CTL, (ADXL_AUTO_SLEEP | ADXL_LINK));   /* auto low power ,deep sleep */
	if ( ret <0 )
		goto err_detect_failed;
	...
	gs->input_dev->id.vendor = GS_ADIX345;
	...
	printk(KERN_INFO "gs_probe: Start ADXL345  in %s mode\n", gs->use_irq ? "interrupt" : "polling");
