...
static void __iomem  *pRegUsbOtgsc = NULL;
#endif
...
#if HUAWEI_HWID_L2(S7, S7201)
static int check_is_usb_connected(void)
{
	int is_usb_connected = 0;

    if(NULL == pRegUsbOtgsc)
    {
        return 0;
    }
    
	if (B_SESSION_VALID & readl(pRegUsbOtgsc)) {
        is_usb_connected = 1;
    } else {
        is_usb_connected = 0;
    }
...
static int lcdc_off(struct platform_device *pdev)
{
    ...
    is_usb_connected = check_is_usb_connected();


	if(is_usb_connected) {       
        is_usb_connect = 1;
		msm_proc_comm(PCOM_GET_UUID_HIGH,&is_screen_on,&is_usb_connect);
    }else {
		is_usb_connect = 0;
		msm_proc_comm(PCOM_GET_UUID_HIGH,&is_screen_on,&is_usb_connect);	
    }

	printk(KERN_ERR "lcdc_on: screen off!,is_usb_connected = %d\n",is_usb_connected);
#endif
...
#if HUAWEI_HWID_L2(S7, S7201)
	int is_usb_connected = 0;
    unsigned is_screen_on = 1, is_usb_connect = 0;
...
#if HUAWEI_HWID_L2(S7, S7201)    
     pRegUsbOtgsc = ioremap(USB_OTGSC, 4);
     if(NULL == pRegUsbOtgsc)
     {
         return -EINVAL;
     }
