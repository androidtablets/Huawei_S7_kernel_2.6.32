...
MODULE_VERSION("1.0");
/* close diag */
static u16 close_diag=0;
static int android_close_diag(const char *buffer, struct kernel_param *kp);
module_param_call(close_diag, android_close_diag, android_close_diag,
					&close_diag, 0664);
MODULE_PARM_DESC(close_diag, "close diag by scsi");
...
static int android_close_diag(const char *buffer, struct kernel_param *kp)
{
	return sprintf(buffer, "%d", close_diag);	
}
void set_pid(void)
{
	#ifdef CLOSE_DIAG_DEBUG
	printk( KERN_ERR  "is_time_out>>>>>>>>>>>>>>>>>>>>>>>>  set_pid:%s \n",&close_diag );	
	#endif
	close_diag=!close_diag;
	#ifdef CLOSE_DIAG_DEBUG
	printk( KERN_ERR  "is_time_out>>>>>>>>>>>>>>>>>>>>>>>>  android_set_pid:%s \n",&close_diag );	
	#endif
...
#else
			ret = mass_storage_function_add(dev->cdev, c, 
							dev->pdata->msc_vendor, dev->pdata->msc_product);
#endif
			if (ret)
				return ret;
			break;
